...
export function GlobalRankingTable() {
  ...
  if (globalRankings.length === 0) {
    return <div className="text-center py-8 text-gray-400">No hay clasificaciones disponibles.</div>;
  }
  ...
  return (
    <div>
      ...
      <div className="flex items-center justify-between px-4 py-3 text-sm text-gray-400">
        <span>Mostrando {globalRankings.length} de {pagination.total} entradas</span>
      </div>
    </div>
  );
}
