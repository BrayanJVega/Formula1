...
export const simulationApi = {
  async runRace(payload: RunRacePayload): Promise<ApiResponse<{ result: RaceSimulationResult }>> {
    const { data } = await apiClient.post('/simulation/race', payload);
    return data;
  },

  async runSeason(payload: RunSeasonPayload): Promise<ApiResponse<{ result: SeasonSimulationResult }>> {
    const { data } = await apiClient.post('/simulation/season', payload);
    return data;
  },

  async runWhatIf(payload: RunWhatIfPayload): Promise<ApiResponse<{ result: { base: RaceSimulationResult; modified: RaceSimulationResult } }>> {
    const { data } = await apiClient.post('/simulation/what-if', payload);
    return data;
  },

  async getHistory(type?: string): Promise<ApiResponse<unknown[]>> {
