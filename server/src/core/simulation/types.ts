...
export interface SimulatedDriverResult {
  ...
  tyreHistory: { lap: number; compound: string }[];
  incidents: Incident[];
  lapTimes: number[];
...
export interface RaceSimulationResult {
  ...
  weatherChanges: { lap: number; condition: WeatherCondition }[];
  laps: number;
  ...
  retirements: number;
  margins: { winner: number; podium: number; points: number };
}
...
export interface SeasonSimulationResult {
  ...
  championshipStandings: { driverId: string; driverName: string; teamId: string; points: number; wins: number; podiums: number; position: number }[];
  constructorStandings: { teamId: string; teamName: string; points: number; wins: number; position: number }[];
}
